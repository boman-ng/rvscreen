...
export RVSCREEN_TASK23_SUMMARY_PATH="${output_dir}/task23_summary.tsv"
export RVSCREEN_TASK23_RSS_PATH="${output_dir}/task23_rss.tsv"
export RVSCREEN_TASK26_SUMMARY_PATH="${output_dir}/task26_summary.tsv"
export CARGO_TERM_COLOR="never"

cd "${repo_root}"

cargo bench --bench task23_parallel_screen 2>&1 | tee "${output_dir}/task23_bench.log"
cargo run --profile bench --bin task23_rss_probe -- "${rss_threads}" 2>&1 | tee "${output_dir}/task23_rss.log"
cargo bench --bench task26_perf 2>&1 | tee "${output_dir}/task26_bench.log"
